...
if( !class_exists('LED_SITE_INDICATOR_Help') ){
	...
	class LED_SITE_INDICATOR_Help {
		...
		function my_plugin_add_help() {
			...
			$broker = '<h3>' . __( 'Setting up the broker', 'led-site-indicator' ) . '</h3>';
			$broker .= '<p>' . __( 'An MQTT broker is a server that distrubutes MQTT messages to subscribed devices.', 'led-site-indicator' ) . ' ';
			$broker .= __( "In order for LED-SITE-INDICATOR to connect to a broker, it needs the following information.", 'led-site-indicator' ) . '</p>';
			$broker .= '<ul>';
			...
			$screen->add_help_tab( array( 'id' => 'mqtt-broker-help', 'title' => __( 'Broker settings', 'led-site-indicator' ), 'content' => $broker ));
			...
			$screen->set_help_sidebar(
				...
				'<a href="http://ledindicator.devsoft.co.za/learn" target="_blank">' . __( 'LED Site Indicator learning', 'led-site-indicator' ) . '</a><br />' .
				'<p><a href="http://wordpress.org/support/" target="_blank">' . __( 'Support Forums', 'led-site-indicator' ) . '</a><br />' .
